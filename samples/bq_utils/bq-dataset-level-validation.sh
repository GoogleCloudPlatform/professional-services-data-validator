...
do
  command="data-validation validate column -sc my_bq_conn_source -tc my_bq_conn_target -rh $5 -tbls $1.$2.$table=$3.$4.$table ${@:6}"
  eval "$command"

  command="data-validation validate schema -sc my_bq_conn_source -tc my_bq_conn_target -rh $5 -tbls $1.$2.$table=$3.$4.$table"
  eval "$command"
done < "$input"
