...
class DataValidation(object):
    ...
    def _execute_validation(self, validation_builder, process_in_memory=True):
        ...
        if process_in_memory:
            ...
            # Impala client returns Nans in place of None, replace for consisntent comparisons
            if self.config_manager._source_conn[consts.SOURCE_TYPE] == "Impala":
                source_df = source_df.replace(numpy.nan, "None")
            if self.config_manager._target_conn[consts.SOURCE_TYPE] == "Impala":
                target_df = target_df.replace(numpy.nan, "None")
