...
def _calculate_difference(field_differences, datatype, validation, is_value_comparison):
    ...
    if isinstance(datatype, ibis.expr.datatypes.Timestamp):
        ...
        source_value = field_differences["differences_source_value"].cast("float64").round(digits=4)
        target_value = field_differences["differences_target_value"].cast("float64").round(digits=4)
    else:
