...
This one-to-one mapping of index to yaml file requires the number of completions/tasks to equal the number of yaml files, which does not scale to a migration of hundreds or thousands of tables where the yaml files are named after the tables rather than numbered sequentially. Therefore, if DVT can also determine the *total* number of tasks in the job, it instead sorts all the yaml files in the config directory by name and deals them out to the tasks round-robin, so that task `i` of `n` tasks runs files `[i::n]` sequentially within its container. The total is read from `JOB_COMPLETION_COUNT` (Kubernetes) or `CLOUD_RUN_TASK_COUNT` (Cloud Run). Note that, unlike the index, Kubernetes does not inject a completion count into the pod, so `JOB_COMPLETION_COUNT` has to be set explicitly in the Job manifest; Cloud Run sets `CLOUD_RUN_TASK_COUNT` automatically. If no valid total is found DVT falls back to the original one-to-one behaviour. In either mode, a config directory holding no yaml files is an error rather than a successful run of no validations; this matters for Cloud Storage where a mistyped prefix, for example `gs://my-bucket/typo-dir/`, simply matches no objects instead of raising a "not found" error.
