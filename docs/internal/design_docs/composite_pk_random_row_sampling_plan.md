...
> Only set `tuple_in_supported() = True` on backends where **BOTH the database engine SQL dialect AND the Ibis backend compiler in DVT** natively compile tuple/struct literals without error. If Ibis compiler support is missing for a backend, `tuple_in_supported()` must default to `False`, safely routing to the universal `OR-of-ANDs` fallback.
...
2. Adding a `tuple_in_supported() -> bool` method to each database backend (returning `True` for engines like PostgreSQL, MySQL, Oracle, Snowflake, and Db2, and `False` by default on SQL Server, Spanner, Teradata, and BigQuery).
3. Extending `FilterField` with `@staticmethod def composite_isin(...)` to check `client.tuple_in_supported()`:
...
class FilterField(object):
    ...
    def composite_isin(
        ...
        if hasattr(client, "tuple_in_supported") and client.tuple_in_supported():
...
- **Path 1: Supported Engines (`client.tuple_in_supported() == True`)**:
