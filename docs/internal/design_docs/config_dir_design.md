...
### Credential Exposure in JSON Configs
JSON configurations generated by DVT (`convert_config_to_json`) are self-contained and embed the **full connection details and credentials** (including passwords, private keys, etc.) in plain text. 

Storing many JSON files in a directory via `--config-dir-json` amplifies this exposure.
*   **Status:** For this phase, this behavior will remain but **must be documented with a prominent warning**.
*   **Future Work:** A separate GitHub issue should be created to implement "shallow" JSON configurations (similar to YAML, storing only connection names) or to support encrypted secrets in JSON configs.
