...
The `--config-dir` flag will specify the directory with the YAML files to be executed in parallel. DVT sorts all YAML files in the config directory by name and each task runs the file corresponding to its task/job index in the sorted list. This supports both partition directories generated by `generate-table-partitions` (where files are numbered `0000.yaml` to `<partition_num - 1>.yaml`, e.g. `gs://my_config_dir/source_schema.source_table/`) and directories containing YAML files named after their contents (e.g. `hr.employees.yaml`).

When creating your Cloud Run Job or Kubernetes Indexed Job, set the number of tasks/completions equal to the number of YAML files in the directory so that each task index maps to a validation file:
*   Cloud Run sets `CLOUD_RUN_TASK_COUNT` automatically from `--tasks`.
*   Kubernetes only injects `JOB_COMPLETION_INDEX` automatically, so you should also set `JOB_COMPLETION_COUNT` in the Job manifest to match `spec.completions`.
