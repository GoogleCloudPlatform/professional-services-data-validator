...
Below is the command syntax for Generating Tabe Partitions for performing Row Validation on large Tables.
The following command splits Row Validation into multiple steps by generating multiple Config files which can then be run in Parallel or independently.
...
`--partition-key`: Partitions are generated by adding filters on this key. If `--partition-key` is not supplied, it defaults to `--primary-keys`.

(Note: As of now, only monotonically increasing key is supported for `--partition-key`.)

`--partition-num`: Number of partitions or Config files to generate. Maximum is 1000. This parameter is truncated in case it exceeds the table count.

`--config-dir`: Directory path to save the config files.
...
data-validation (--verbose or -v) (--log-level or -ll) generate-table-partitions
  ...
  [--threshold THRESHOLD, -th THRESHOLD]
                        Float max threshold for percent difference
  [--filters SOURCE_FILTER:TARGET_FILTER]
                        Colon spearated string values of source and target filters.
                        If target filter is not provided, the source filter will run on source and target tables.
                        See: *Filters* section
  [--use-random-row, -rr]
                        Finds a set of random rows of the first primary key supplied.
  [--random-row-batch-size RANDOM_ROW_BATCH_SIZE, -rbs RANDOM_ROW_BATCH_SIZE]
                        Row batch size used for random row filters (default 10,000).
  ...
  [--labels LABELS, -l LABELS]
                        Key value pair labels for validation run
  [--service-account SERVICE_ACCOUNT, -sa SERVICE_ACCOUNT]
                        Path to SA key file for result handler output
  [--format FORMAT, -fmt FORMAT]
                        Set the format for printing command output, Supported formats are (text, csv, json, table). Defaults to table
  [--filter-status FILTER_STATUS, -fs FILTER_STATUS]
                        Comma separated list of statuses to filter the validation results. Supported statuses are (success, fail). If no list is
                        provided, all statuses are returned
```
#### Schema Validations
