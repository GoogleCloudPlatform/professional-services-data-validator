...
def test_count_distinct_with_filter(alltypes):
    expr = alltypes.float_col.nunique(
        where=alltypes.float_col.cast('int64') > 1
    )
    result = expr.execute()
...
def test_scalar_param_double(alltypes, df):
    ...
    expected = (
        df.loc[df.double_col == double_value]
        .sort_values('id')
        .reset_index(drop=True)
    )
    tm.assert_frame_equal(result, expected)
...
def test_exists_database(client, name, expected):
...
def test_students_table_schema(students):
    assert students.schema() == ibis.schema(
        [('id', dt.int64), ('name', dt.string), ('division',dt.int64), ('marks',dt.int64), ('exam', dt.string), ('overall_pointer',dt.float64), ('date_of_exam',dt.timestamp)]
    )
